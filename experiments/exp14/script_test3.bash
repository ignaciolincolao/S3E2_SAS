...
do 
    ...
    do
        mkdir -p "../save_exp3/$len2"
        for rate in "${coolingrates[@]}"
        do
            mkdir -p "../save_exp3/$len2/rate_$rate"
            for temp_ini in "${temp_inital[@]}"
            do
                mkdir -p "../save_exp3/$len2/rate_$rate/$temp_ini"
                for reheating in "${reheatinRates[@]}"
                do
                    mkdir -p "../save_exp3/$len2/rate_$rate/$temp_ini/$reheating"
                    for e_const in "${e_constRates[@]}"
                    do
                        mkdir -p "../save_exp3/$len2/rate_$rate/$temp_ini/$reheating/$e_const"
                        ...
                        ./s3e2_sas $temp_ini 0.00000009 $seed 15 30 25 $rate $reheating $e_const 2537505 1 $len2 -1 -1 -1 -1 -1 "../save_exp3/$len2/rate_$rate/$temp_ini/$reheating/$e_const/" "$repeat" "exp14"
                    done
                done
            done
        done
